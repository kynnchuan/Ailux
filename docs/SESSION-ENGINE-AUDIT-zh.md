...
>
> **修订（v3）**：问题 1/3/4/5/6 全部完成实施，每条记录对应 commit；详见"修复总览（v3）"小节。
...
| # | 状态 | 实施 commit | 主要落地 |
|---|:---:|---|---|
| 1 | ✅ 已修复 | `4c239b3` | `history` 改用单把 `ReentrantLock` 守护读写；`turnLock` 改用 `kotlinx.coroutines.sync.Mutex` 串行 turn；新增 `*SessionAdapterSnapshotRaceTest` / `StatelessProviderSessionTest#snapshotConcurrentWithStreamGenerate_neverThrows` 并发回归 |
| 5 | ✅ 已修复 | `53b0265` | `CharClassTokenEstimator` 独立 top-level（避开 JDK 21 编译的 AAR 在 JDK 17 单测 runner 上 `UnsupportedClassVersionError`）；CJK 段 ~1.5 char/token，Latin 段 ~4 char/token |
| 6 | ✅ 已修复 | `e77f173` | `SessionConfig.modelId` 路径 A；`Session.modelId` 默认实现；`SessionDefaults.collectToResponse` 把 modelId 写入 `LLMResponse.model`；`LocalRuntimeProvider` 派生 `local:<stem>` 并加 `LLMRequest.model` 失配校验（`MODEL_NOT_FOUND`） |
| 4 | ✅ 已修复 | `1c60a11` | `LocalRuntimeConfig.maxOutputTokens` 引擎级硬护栏 → 透传 `EngineConfig.maxNumTokens`；KDoc 澄清与 `LLMRequest.maxTokens` 的语义差（引擎级 vs 消费端），云端 OpenAI/Anthropic Mapper 已正确映射 maxTokens（无需改云端） |
| 3 | ✅ 已修复 | `9bb4216` | `EngineSession.cancel()` SPI（默认空）+ `LiteRTLMSession.cancel() → conversation.cancelProcess()`；`LiteRTLMEngine.capabilities.supportsInterruptibleCancellation = true`；两个适配层改 `channelFlow + worker Job + inflightWorker?.cancelAndJoin()`，CANCEL_PREVIOUS / 消费端 flow 取消两条路径都触发 native abort；半成品 assistant reply 不污染 history；新增 `LocalEngineSessionAdapterCancelTest` 与 `StatelessProviderSessionTest` 各 2 个 case 验证真取消 + 不污染 history |

四模块 `:ailux-core` / `:ailux-provider-local` / `:ailux-runtime` / `:ailux-runtime-litertlm` `testDebugUnitTest` 全部 BUILD SUCCESSFUL。
...
## 问题 1 — snapshot/close 与 streamGenerate 锁不一致 ✅ 真 bug · 已修复 (`4c239b3`)
...
## 问题 3 — CANCEL_PREVIOUS 只是 best-effort ⚠️ 部分成立 + 关键能力升级 · 已修复 (`9bb4216`)
...
## 问题 4 — maxTokens 软上限兜底 ⚠️ P5 / 可选（v2 修订）· 已修复 (`1c60a11`)
...
## 问题 5 — sizeInTokens 中文低估 ✅ 真问题 · 已修复 (`53b0265`)
...
## 问题 6 — LLMResponse 缺少 model 名 ✅ 真问题 · 已修复 (`e77f173`)
